...
out_file_name = "europ_media.txt"
out_file = open(out_file_name, 'w')

taxon_id_lookup = eval(open('europ_taxa_dict.txt').read())
...
for line in in_file:
	...
	if taxon.find('?') != -1:
		...
		description = row[6]
	owner = row[7]
	out_file.write('\t'.join([mediaid,taxon_id,type,format,access_uri,license,publisher,contributor,creator,furtherinfourl,description,owner + '\n']))
